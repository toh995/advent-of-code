...
def main():
    ...
    part1_answer = part1(grid, lengths)
...
def get_grid():
    ...
    for i in range(len(rows)):
        ...
        for j in range(len(row)):
            cube = row[j]
            grid[(x_min+j, y_min+i, 0)] = cube

    return grid, (x_length, y_length, z_length)


def part1(grid, lengths):
...
def get_neighbor(grid, start_point, delta_tuple):
    x, y, z = start_point
    x_delta, y_delta, z_delta = delta_tuple

    new_point = (x + x_delta, y + y_delta, z + z_delta)
