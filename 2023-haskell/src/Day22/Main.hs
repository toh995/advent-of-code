...
import Data.Functor ((<&>))
import Data.List (singleton, sort, sortOn)
import Data.Map (Map)
...
part1 bricks =
  ...
  where
    Graph{onTopOfMap, belowMap} = newGraph bricks
    canDisintegrate Brick{brickId} =
        all
            ( \topId ->
                case onTopOfMap M.!? topId of
                    Just (_ : _ : _) -> True
                    _ -> False
            )
            topIds
  ...
  where
    sortedBricks =
        sortOn
            ...
            bricks
    go [] _ _ onTopOfMap belowMap = Graph{onTopOfMap, belowMap}
    go (brick : otherBricks) highestZs highestIds onTopOfMap belowMap =
        go
            otherBricks
            nextHighestZs
            nextHighestIds
      ...
      where
        ...
        nextOnTopOfMap =
            foldr
                (M.insertWith (++) (brickId brick) . singleton)
                onTopOfMap
                belowIds
        nextBelowMap =
            foldr
                (\belowId -> M.insertWith (++) belowId [brickId brick])
                belowMap
                belowIds
        belowIds =
            toList
...
data Graph = Graph
    { onTopOfMap :: Map BrickId [BrickId]
    , belowMap :: Map BrickId [BrickId]
    }
