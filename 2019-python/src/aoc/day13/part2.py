...
def run_game(stdscr: curses.window, program: list[int]) -> int:
    ...
    stdscr.addstr(
        1, 0, "Controls: A/D or Arrow Keys to move, S or Down to stay, Q to quit"
    )

    # Phase 1: Initial render - collect all tiles until we have ball and paddle
    score = do_initial_render(stdscr, program_executor, score)

    # Phase 2: Game loop - block for input and process one update at a time
    return do_game_loop(stdscr, program_executor, score, input_)


def do_initial_render(
    stdscr: curses.window, program_executor: Iterator[int], score: int
) -> int:
    game_tiles = {}
    ball_draw_count = 0
    has_paddle = False
    ...
    while True:
        ...
        if (x, y) == (-1, 0):
            ...
            # Exit on second ball draw (and we have paddle)
            if ball_draw_count >= 2 and has_paddle:
                draw_entire_screen(stdscr, game_tiles)
                stdscr.refresh()
                return score
...
def do_game_loop(
    ...
    program_executor: Iterator[int],
    score: int,
    input_: list[int],
) -> int:
    stdscr.nodelay(False)  # Blocking input for game loop

    while True:
        # Process one game update
        try:
            x = next(program_executor)
            y = next(program_executor)
            third_item = next(program_executor)
        ...
        if (x, y) == (-1, 0):
            ...
            tile = Tile(third_item)
            draw_tile(stdscr, x, y, tile)

            # Only ask for user input when drawing a ball
            if tile == Tile.BALL:
...
def draw_entire_screen(stdscr: curses.window, game_tiles: dict[Coord, Tile]) -> None:
    for (x, y), tile in game_tiles.items():
        draw_tile(stdscr, x, y, tile)


def draw_tile(stdscr: curses.window, x: int, y: int, tile: Tile) -> None:
    char = TILE_TO_STR[tile]
    try:
        stdscr.addstr(y + 3, x, char)  # Offset by 3 to leave room for score/controls
    except curses.error:
        pass  # Ignore if we can't draw (screen too small)


def part2(program: list[int]) -> int:
    return curses.wrapper(run_game, program)
